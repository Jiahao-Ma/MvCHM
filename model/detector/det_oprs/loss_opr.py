...
def softmax_loss(score, label, ignore_label=-1):
    ...
    vlabel = label * mask
    onehot = torch.zeros(vlabel.shape[0], crowdet_config.num_classes, device=score.device)
...
def emd_loss_softmax(p_b0, p_s0, p_b1, p_s1, targets, labels):
    ...
    fg_masks = labels > 0
    # multiple class
    pred_delta = pred_delta.reshape(-1, crowdet_config.num_classes, 4)
    ...
    localization_loss = smooth_l1_loss(
        pred_delta,
        targets[fg_masks],
        crowdet_config.rcnn_smooth_l1_beta)
...
def emd_loss_focal(p_b0, p_s0, p_b1, p_s1, targets, labels):
    ...
    objectness_loss = focal_loss(pred_score, labels,
            crowdet_config.focal_loss_alpha, crowdet_config.focal_loss_gamma)
    fg_masks = (labels > 0).flatten()
    localization_loss = smooth_l1_loss(
            pred_delta[fg_masks],
            targets[fg_masks],
            crowdet_config.smooth_l1_beta)
