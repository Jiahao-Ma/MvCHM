...
from model.detector.det_oprs.bbox_opr import box_overlap_opr, bbox_transform_opr
...
def retina_anchor_target(anchors, gt_boxes, im_info, top_k=1):
    ...
    for bid in range(crowdet_config.train_batch_per_gpu):
        ...
        labels = labels * (max_overlaps >= crowdet_config.negative_thresh)
        ignore_mask = (max_overlaps < crowdet_config.positive_thresh) * (
                max_overlaps >= crowdet_config.negative_thresh)
        ...
        if crowdet_config.allow_low_quality:
    ...
    if crowdet_config.train_batch_per_gpu == 1:
