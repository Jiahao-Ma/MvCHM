...
class GaussianKernel(object):
    ...
    def gaussian_kernel_heatmap(self, heatmaps, box_cx, box_cy):
        heatmaps[int(box_cy), int(box_cx)] = 1
        return heatmaps
    ...
    def load_from_file(self):
        try:
            batch_gt = np.load(self.save_dir)
            return batch_gt
        except:
            print('\033[31mRGK load error.\033[0m')
    ...
    def dump_to_file(self):
        self.generate()
        # self.viz_gk()
        self.heatmaps = np.array(self.heatmaps, dtype=np.float32)
        
        try:
            np.savez(self.save_dir, heatmaps=self.heatmaps)
            print('RGK has been save %s' %self.save_dir)
        except:
            print('\033[31mRGK save error.\033[0m')
        return {'heatmaps':self.heatmaps}
